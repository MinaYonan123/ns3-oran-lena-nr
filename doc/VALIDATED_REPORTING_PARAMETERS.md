# Validated Reporting Parameters — 5G-LENA (NR Module)

This document validates every KPI listed in [`REPORTING_PARAMETERS_AND_3GPP_KPIS.md`](REPORTING_PARAMETERS_AND_3GPP_KPIS.md) against the current **5G-LENA** codebase under `src/nr/`.  
Validation date: codebase inspection of `mmwave-LENA-oran` NR module (includes O-RAN/E2/CCC fork extensions in `nr-gnb-net-device.cc`).
...
- “Manual connect” KPIs are still **IMPLEMENTABLE** if the `TraceSource` is registered on the model class.
- Simulation models are **approximate** relative to TS 28.552; status reflects *availability*, not 3GPP bit-exact compliance.
- E2/KPM items are validated on the **ns-3 measurement path**; encoding into ASN.1 is done by `LenaIndicationMessageHelper` (external to `src/nr/`, included from the O-RAN interface).
